...
@Repository
public class RetroalimentacionDaoImpl implements IRetroalimentacionDAO{
	...
	public List<Trabajador> obtenerCorreosEvaluadosRetroalimentacionPendiente(String codCalendario) throws SQLException {
		...
		this.jdbcCall = new SimpleJdbcCall(this.jdbcTemplate)
				...
				.withCatalogName(DBConstants.PACKAGE_SISTEMA).withProcedureName(DBConstants.PROCEDURE_CORREO_EVALUADO_RETROALIMENTACION)
	...
	public List<Trabajador> obtenerCorreosEvaluadoresRetroalimentacionPendiente(String codCalendario) throws SQLException {
		...
		this.jdbcCall = new SimpleJdbcCall(this.jdbcTemplate)
				.withSchemaName(DBConstants.SCHEMA_NAME)
				.withCatalogName(DBConstants.PACKAGE_SISTEMA).withProcedureName(DBConstants.PROCEDURE_CORREO_EVALUADOR_RETROALIMENTACION)
				.withoutProcedureColumnMetaDataAccess().declareParameters(						
						new SqlParameter("p_codigo_calendario", Types.VARCHAR),
						new SqlOutParameter("CUR_DATOS", OracleTypes.CURSOR, new ColumnMapRowMapper()),
						new SqlOutParameter("o_mensaje", Types.VARCHAR),
